var daily_spending = {
    ...
    queryEventByMonth: 'select t1.*, t2.color, t2.text_color from `daily_spending` as t1, `spending_type` as t2 where (t1.date between ? and ?) and (t1.type = t2.id) and (t1.userid = ?)',
    queryEventOption: 'select * from spending_type',
    deleteEventById: 'delete from daily_spending where id=?',
